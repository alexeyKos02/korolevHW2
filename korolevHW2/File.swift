...
extension UIView {
    ...
    func pin(to superview: UIView, _ sides: [PinSide], _ number : [Int]) {
        for i in 0...(number.count-1){
            switch sides[i]{
            case .top:
                pinTop(to: superview, number[i])
            case .bottom:
                pinBottom(to: superview, number[i])
            case .left:
                pinLeft(to: superview, number[i])
            case .right:
                pinRight(to: superview, number[i])
            }
        }
    }
